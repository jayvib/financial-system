...
type authFunc func([]byte) (*oauth2.Config, error)

// Retrieve a Token_path, saves the Token_path, then returns the generated client.
func NewClient(reader io.Reader, fn authFunc) (*http.Client, error) {
...
// Retrieve a Token_path, saves the Token_path, then returns the generated client.
func getClient(oconf *oauth2.Config) *http.Client {
	conf, _ := config.DefaultConfig()
	tok, err := tokenFromFile(conf.Path.TokenFile)
	if err != nil {
		tok = getTokenFromWeb(oconf)
		saveToken(conf.Path.TokenFile, tok)
	}
	return oconf.Client(context.Background(), tok)
}
